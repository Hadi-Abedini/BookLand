...
function Categorie() {
  ...
  useEffect(() => {
    if (!ID && categories) {
      const firstCategory = categories?.data?.data?.categories?.[0];
      if (firstCategory) {
        navigate(`/categorie/${firstCategory._id}/category`);
      }
    }
  ...
  return (
    <div className="h-full w-full flex gap-8 px-8 py-10 bg-[#E8F4E8]">
